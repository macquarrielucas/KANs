...
function lotka!(du,u,p,t) 
    du[1] = u[1] - 0.5f0*u[1]*u[2]
    du[2] = 0.5f0*u[1]*u[2] - u[2]
end
...
function loss_train(p)
    mean(abs2, Xn[:, 1:end_index] .- CuArray(predict(p)))
end

function loss_test(p) 
    mean(abs2, Xn .- CuArray(predict_test(p)))
end
...
@time for i in iterator
    ...
    grad = Zygote.gradient(loss_train, pM)[1]
    ...
    # CALLBACK
    loss_curr=deepcopy(loss_train(pM))
    loss_curr_test=deepcopy(loss_test(pM))
    set_description(iterator, string("Iter:" ,i, "| Loss:", @sprintf("%.2f", loss_curr), "|",
                            "Test_Loss:", @sprintf("%.2f", loss_curr_test), "|"))

    if i%10 == 0
        @time @allowscalar display(save_training_frame(pM,i, training_dir; save=true))
    end
    # SAVE
    #callback(i)
end